...
load_current_value do |new_resource|
  ...
  storage_latest_policy response['storage']['latestPolicy']
  ...
  puts "MISCHA: boxcutter_nexus_docker_hosted_repository::load_current_value #{response}"
end
...
action :create do
  repository_exist = Boxcutter::Sonatype::Resource::Helpers.repository_exist?(new_resource, 'docker', 'hosted')
  puts "MISCHA boxcutter_nexus_docker_hosted_repository::create repository_exist=#{repository_exist}"
  unless repository_exist
    converge_if_changed do
      Boxcutter::Sonatype::Resource::Helpers.repository_create(new_resource, 'docker', 'hosted')
    end
  end
end
...
action :update do
  repository_exist = Boxcutter::Sonatype::Resource::Helpers.repository_exist?(new_resource, 'docker', 'hosted')
  puts "MISCHA boxcutter_nexus_docker_hosted_repository::update repository_exist=#{repository_exist}"
  unless repository_exist
  ...
  converge_if_changed(
    ...
    Boxcutter::Sonatype::Resource::Helpers.repository_update(new_resource, 'docker', 'hosted')
  end
end

action :delete do
  repository_exist = Boxcutter::Sonatype::Resource::Helpers.repository_exist?(new_resource, 'docker', 'hosted')
  puts "MISCHA boxcutter_nexus_docker_hosted_repository::delete repository_exist=#{repository_exist}"
  if repository_exist
