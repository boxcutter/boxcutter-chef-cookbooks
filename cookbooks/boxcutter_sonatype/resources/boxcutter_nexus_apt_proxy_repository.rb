unified_mode true
provides :boxcutter_nexus_apt_proxy_repository
...
property :http_client_connection_timeout, Integer,
         description: 'Seconds to wait for activity before stopping and retrying ' \
                      'the connection. Leave blank to use the globally defined HTTP timeout.'
...
property :apt_distribution, String, default: 'noble',
         description: 'Distribution to fetch e.g. bionic'
property :apt_flat, [TrueClass, FalseClass], default: false

load_current_value do |new_resource|
  response = nil
  begin
    response = Boxcutter::Sonatype::Resource::Helpers.repository_get(new_resource, 'apt', 'proxy')
  ...
  apt_flat response['apt']['flat']
  puts "MISCHA: boxcutter_nexus_apt_proxy_repository::load_current_value #{response}"
end
...
action :create do
  repository_exist = Boxcutter::Sonatype::Resource::Helpers.repository_exist?(new_resource, 'apt', 'proxy')
  puts "MISCHA boxcutter_nexus_apt_proxy_repository::create repository_exist=#{repository_exist}"
  unless repository_exist
    converge_if_changed do
      Boxcutter::Sonatype::Resource::Helpers.repository_create(new_resource, 'apt', 'proxy')
    end
  end
end
...
action :update do
  repository_exist = Boxcutter::Sonatype::Resource::Helpers.repository_exist?(new_resource, 'apt', 'proxy')
  puts "MISCHA boxcutter_nexus_apt_proxy_repository::update repository_exist=#{repository_exist}"
  unless repository_exist
  ...
  converge_if_changed(
    ...
    Boxcutter::Sonatype::Resource::Helpers.repository_update(new_resource, 'apt', 'proxy')
  end
end

action :delete do
  repository_exist = Boxcutter::Sonatype::Resource::Helpers.repository_exist?(new_resource, 'apt', 'proxy')
  puts "MISCHA boxcutter_nexus_apt_proxy_repository::delete repository_exist=#{repository_exist}"
  if repository_exist
