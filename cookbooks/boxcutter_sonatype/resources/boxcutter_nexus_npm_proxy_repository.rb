unified_mode true
provides :boxcutter_nexus_npm_proxy_repository
...
property :npm_remove_quarantined, [TrueClass, FalseClass], :default => false

load_current_value do |new_resource|
  response = nil
  begin
    response = Boxcutter::Sonatype::Resource::Helpers.repository_get(new_resource, 'npm', 'proxy')
  ...
  npm_remove_quarantined response['npm']['removeQuarantined']
  puts "MISCHA: boxcutter_nexus_npm_proxy_repository::load_current_value #{response}"
end
...
action :create do
  repository_exist = Boxcutter::Sonatype::Resource::Helpers.repository_exist?(new_resource, 'npm', 'proxy')
  puts "MISCHA boxcutter_nexus_npm_proxy_repository::create repository_exist=#{repository_exist}"
  unless repository_exist
    converge_if_changed do
      Boxcutter::Sonatype::Resource::Helpers.repository_create(new_resource, 'npm', 'proxy')
    end
  end
end
...
action :update do
  repository_exist = Boxcutter::Sonatype::Resource::Helpers.repository_exist?(new_resource, 'npm', 'proxy')
  puts "MISCHA boxcutter_nexus_npm_proxy_repository::update repository_exist=#{repository_exist}"
  unless repository_exist
  ...
  converge_if_changed(
    ...
    Boxcutter::Sonatype::Resource::Helpers.repository_update(new_resource, 'npm', 'proxy')
  end
end

action :delete do
  repository_exist = Boxcutter::Sonatype::Resource::Helpers.repository_exist?(new_resource, 'npm', 'proxy')
  puts "MISCHA boxcutter_nexus_npm_proxy_repository::delete repository_exist=#{repository_exist}"
  if repository_exist
