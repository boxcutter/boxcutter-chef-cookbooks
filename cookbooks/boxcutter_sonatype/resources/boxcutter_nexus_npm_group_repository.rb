...
load_current_value do |new_resource|
  response = nil
  begin
    response = Boxcutter::Sonatype::Resource::Helpers.repository_get(new_resource, 'npm', 'group')
  ...
  puts "MISCHA: boxcutter_nexus_npm_group_repository::load_current_value #{response}"
end
...
action :create do
  repository_exist = Boxcutter::Sonatype::Resource::Helpers.repository_exist?(new_resource, 'npm', 'group')
  puts "MISCHA boxcutter_nexus_npm_group_repository::create repository_exist=#{repository_exist}"
  unless repository_exist
    converge_if_changed do
      Boxcutter::Sonatype::Resource::Helpers.repository_create(new_resource, 'npm', 'group')
    end
  end
end
...
action :update do
  ...
  converge_if_changed(
    ...
    Boxcutter::Sonatype::Resource::Helpers.repository_update(new_resource, 'npm', 'group')
  end
end
