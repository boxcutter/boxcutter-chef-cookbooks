...
-----END PGP PUBLIC KEY BLOCK-----
  ...
  node.default['fb_apt']['repos'] <<
    "deb https://packages.treasuredata.com/lts/5/ubuntu/#{node['lsb']['codename']}/ " +
...
execute 'install fluent-plugin-grafana-loki' do
  command '/opt/fluent/bin/fluent-gem install fluent-plugin-grafana-loki'
  not_if '/opt/fluent/bin/fluent-gem list | grep fluent-plugin-grafana-loki'
end

file '/etc/fluent/fluentd.conf' do
  action :delete
end

file '/etc/default/fluentd' do
  content(
    'FLUENT_PACKAGE_OPTIONS="-c /etc/fluent/fluentd.yaml"',
  )
...
service 'fluentd' do
  only_if { node['boxcutter_fluent']['fluentd']['enable'] }
  action [:enable, :start]
end

service 'disable fluentd' do
  service_name 'fluentd'
  not_if { node['boxcutter_fluent']['fluentd']['enable'] }
  action [:stop, :disable]
end
